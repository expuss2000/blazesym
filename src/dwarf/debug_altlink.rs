...
pub(crate) fn read_debug_altlink(parser: &ElfParser) -> Result<Option<(&OsStr, &[u8])>> {
...
fn parse_debug_altlink_section_data(mut data: &[u8]) -> Result<Option<(&OsStr, &[u8])>> {
    let file = data
        .read_cstr()
...
mod tests {
    ...
    fn unaligned_debug_altlink_parsing() {
        let section_data = [
            b'.', b'.', b'/', b'.', b'.', b'/', b'p', b'r', b'o', b'g', b'r', b'a', b'm', 0x0,
        ...
        assert_eq!(file, OsStr::new("../../program"));
        assert_eq!(
            build_id,
            [
                0x7f, 0xd3, 0x76, 0x0a, 0xf3, 0x98, 0xa1, 0xcc, 0x3f, 0x90, 0x45, 0x69, 0x9a, 0xda,
